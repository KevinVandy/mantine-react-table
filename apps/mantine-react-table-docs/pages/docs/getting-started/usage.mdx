...
Here is a brief overview of how to use Mantine React Table. This is a very simple example, and will not cover all features of the library, but it is a great starting point.

### Setup

To start using MantineReactTable, you first need to [install](/docs/getting-started/install) `mantine-react-table` and the necessary Mantine UI dependencies.
...
If you need to change the colors of the table components, advanced [Mantine UI Theming](https://mantine.dev/theming/mantine-provider/#nested-mantineproviders) is covered in the [Customize Components](/docs/guides/customize-components) guide.
...
If you are using TypeScript, you may also want to import some TypeScript types from `'mantine-react-table'` to take advantage of the advanced strong type checking and autocompletion features. The `MRT_ColumnDef` type is especially useful for defining your columns in a type-safe way.

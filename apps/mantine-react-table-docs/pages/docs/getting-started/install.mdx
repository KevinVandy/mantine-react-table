...
> `mantine-react-table` requires **Material UI V5** packages as dependencies in your project.
>
...
> 1.  `@mantine/core` (v5)
> 2.  `@mantine/hooks` (v5)
> 3.  `@tabler/icons` (> v1.118)
> 4.  `@emotion/react` (v11)
> 5.  `react` and `react-dom` (v17 or v18)
...
```bash
npm install mantine-react-table @mantine/core @mantine/hooks @tabler/icons @emotion/react
```

PNPM

```bash
pnpm add mantine-react-table @mantine/core @mantine/hooks @tabler/icons @emotion/react
```

Yarn

```bash
yarn add mantine-react-table @mantine/core @mantine/hooks @tabler/icons @emotion/react
```

> You do NOT need to install `@tanstack/react-table`, as it is already an internal dependency of `mantine-react-table`.
>
...
<FAQs
  faqStructuredData={{
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: 'Why is React 17 or 18 required for Mantine React Table?',
        acceptedAnswer: {
          '@type': 'Answer',
          text: '<p>React 17 or later is required by Material UI v5 itself. Some event listeners on MUI TextField components will not work with React 16.x.</p>',
        },
      },
      {
        '@type': 'Question',

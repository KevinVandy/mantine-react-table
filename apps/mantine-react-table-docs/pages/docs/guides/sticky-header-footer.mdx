...
<Head>
  <title>Sticky Header/Footer Feature Guide - Mantine React Table Docs</title>
  <meta
    name="description"
    content="How to use and customize the sticky header feature in Mantine React Table"
  />
</Head>
...
When the sticky header is enabled, you will probably also want to give the table a maxHeight so that the table can scroll vertically, and keep the header visible. You can do this by styling the table container with the `mantineTableContainerProps` prop.
...
```tsx
<MantineReactTable columns={columns} data={data} enableStickyFooter />
```
...
View Extra Storybook **[Examples](https://www.mantine-react-table.dev/?path=/story/styling-sticky-header-examples)**

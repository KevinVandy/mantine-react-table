...
<StateOptionsTable onlyOptions={new Set(['editingCell', 'editingRow'])} />

### Enable Editing

To enable editing, first you just need to set the `enableEditing` prop to `true`.

```jsx
<MantineReactTable columns={columns} data={data} enableEditing={true} />
```
...
const columns = [
  {
    ...
    header: 'Age',
    mantineEditTextInputProps: {
      error: !!validationErrors.age, //highlight Mantine text field red error color

...
A common use for row drag and drop is to allow users to reorder rows in a table. This can be done by setting the `enableRowOrdering` prop to `true`, and then setting up an `onDragEnd` event handler on the `mantineRowDragHandleProps` prop.
...
The Drag and Drop features are not limited to just internally within the same table. You can use them to drag rows to other UI in your application, or even to other tables. This can be done by setting the `enableRowDragging` prop to `true`, and then setting up an `onDragEnd` event handler on the `mantineRowDragHandleProps` prop to perform whatever logic you want to happen when a row is dropped.

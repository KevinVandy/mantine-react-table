...
By default, the `row.id` for each row in the table is simply the index of the row in the table. You can override this and tell Mantine React Table to use a more useful Row ID with the `getRowId` prop. For example, you may want some like this:
...
The selection checkboxes can be customized with the `mantineSelectCheckboxProps` prop. Any prop that can be passed to a Mantine Checkbox component can be specified here. For example, you may want to use a different color for the checkbox, or use some logic to disable certain rows from being selected.

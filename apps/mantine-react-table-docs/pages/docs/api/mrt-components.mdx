...
<Head>
  <title>{'MRT Components - Mantine React Table V2 Docs'}</title>
  <meta
    name="description"
...
In previous versions of `mantine-react-table`, you only had the option to use the `<MantineReactTable />` component. Now in version 2, all internal MRT components are exported for you to to optionally use to build up your own custom table components.
...
It accepts table options as props, just like in v1, but it also now simply accepts the `table` instance as its only prop instead. You have the choice to use the `useMantineReactTable` hook to create the table instance in your own scope or not. Using the `table` prop with the `useMantineReactTable` hook is recommended, because it will give you more flexibility as your table features get more complex.
...
//...
return <MantineReactTable columns={columns} data={data} enableRowPinning />; //passing props instead like in v1 is possible
```

> Do not pass both a `table` prop and other table options as props. Additional table options passed to the `<MantineReactTable />` component will be ignored if a `table` prop is passed. Either specify all table options in the `useMantineReactTable` hook or pass all of them as props to the `<MantineReactTable />` component.

### MRT\_\* Sub Components

Mantine React Table now exports all of its internal components. You can use them for your own use cases.
...
The `<MRT_TablePaper />` component is the outermost component of the table. Using this is pretty much the same thing as using the `<MantineReactTable />` component, except that you cannot pass table options as props to it.
